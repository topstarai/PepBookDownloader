...
3. 根据第1步的思路，先分析分类目录的页面的html代码结构情况，用第二步选择的类库来实现获取大分类目录及其下各学科页面网址，返回结果用Dictionary<string,List<string>>来存放，其中，key表示小学，初中，高中这些大的分类名称，List<string>表示大分类下各学科的页面地址。 具体实现代码如下：
...
4.  迭代第3步所示结果，从各个学科页面内容中进行电子书地址提取。具体代码如下：
